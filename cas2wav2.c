...
//#define OUTPUT_FREQUENCY  22050
//#define OUTPUT_FREQUENCY  21600
uint32_t OUTPUT_FREQUENCY = 44100;

/* default output baudrate */
int BAUDRATE = 3675;

/* digital output or sinusoidal */

int squared = 1;					// ouput signal is squared
...
//  BIGENDIANLONG(OUTPUT_FREQUENCY),
  BIGENDIANLONG(44100),					// Este valor por defecto se machaca con -f , p.e -f 48000
  BIGENDIANLONG(44100),
...
void writePulse(FILE *output,uint32_t f)
{
  ...
  if (squared) {
  	for (n=0;n<(uint32_t)(length/2);n++)			
		putc( 230, output);	 	 	 	 	 	 	 // 255
  	for (n=(uint32_t)(length/2);n<length;n++)
    	putc( -230, output);						// -255
  	...
  	for (n=0;n<(uint32_t)length;n++){
		...
		//printf("length=%f scale=%f value=%d \n", length, scale, (char)(sin((double)n*scale)*127)^128 );	
	}
...
{
  printf("usage: %s [-5] [-s seconds] <ifile> <ofile>\n"
         " -1   use 1200 baud as output baudrate and 48 KHz\n"  
         " -2   use 2400 baud as output baudrate and 48 KHz\n"
         " -3   use 3150 baud as output baudrate and 44,1 KHz\n"
         " -4   use 3600 baud as output baudrate and 44,1 KHz\n"
         " -5   use 3675 baud as output baudrate and 44,1 KHz\n"	  	 
         " -b   define output baudrate (3675)\n"
         " -f   define output frecuency in Hz (44100), 48000, ..\n"
         " -g   define gap time (in seconds) between blocks (default 2)\n"		 
         " -s   force sinusoidal, default output is squared\n"	  	   	       	    	   
	 ,progname);
}
    ...
    if (argv[i][0]=='-') {
      ...
      for(j=1;j && argv[i][j]!='\0';j++)
        ...
        case '1': BAUDRATE=1200; 
				  waveheader.nSamplesPerSec = BIGENDIANLONG(48000);
				  waveheader.nAvgBytesPerSec = BIGENDIANLONG(48000);		
				  break;
        ...
        case '3': BAUDRATE=3150;
				  waveheader.nSamplesPerSec = BIGENDIANLONG(44100);
				  waveheader.nAvgBytesPerSec = BIGENDIANLONG(44100);		
				  break;
				  	     	 
        case '4': BAUDRATE=3600;
				  waveheader.nSamplesPerSec = BIGENDIANLONG(44100);
				  waveheader.nAvgBytesPerSec = BIGENDIANLONG(44100);		
				  break;
				  	     	    	 
        case '5': BAUDRATE=3675;
				  waveheader.nSamplesPerSec = BIGENDIANLONG(44100);
				  waveheader.nAvgBytesPerSec = BIGENDIANLONG(44100);		
				  break;
				  	     	 
		case 'b': BAUDRATE=atof(argv[++i]); j=-1; break;
		
		case 'f': OUTPUT_FREQUENCY = atof(argv[++i]);	   
				  waveheader.nSamplesPerSec = BIGENDIANLONG(OUTPUT_FREQUENCY);
				  waveheader.nAvgBytesPerSec = BIGENDIANLONG(OUTPUT_FREQUENCY);
				  j=-1; break;
        case 'g': stime=atof(argv[++i]); j=-1; break;
		
        case 's': squared=0; break;		
