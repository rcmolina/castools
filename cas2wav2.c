...
void showUsage(char *progname)
{
  printf("usage: %s [-5] [-s seconds] <ifile> <ofile>\n"
         " -1   use 1200 baud as output baudrate\n"  
         " -2   use 2400 baud as output baudrate\n"
         " -3   use 3150 baud as output baudrate\n"
         " -4   use 3600 baud as output baudrate\n"
         " -5   use 3675 baud as output baudrate\n"		 	
  ...
  for (i=1; i<argc; i++) {
    ...
    if (argv[i][0]=='-') {
      ...
      for(j=1;j && argv[i][j]!='\0';j++)
        ...
        case '3': BAUDRATE=3150; break;
		
        case '4': BAUDRATE=3600; break;
		   	   
        case '5': BAUDRATE=3675; break;
