...
uint32_t OUTPUT_FREQUENCY = 21600;
...
/*    if ((length/2-n) >0) putc( (char)127^128, output);
	else putc( (char)-127^128, output); */
	
    if (n < length/2) putc( 255, output);
	else if ((length %2 != 0) && (n == length/2)) putc (128, output);
	else putc( 0, output);	   
}
...
void showUsage(char *progname)
{
  printf("usage: %s [-2] [-s seconds] <ifile> <ofile>\n"
         ...
         " -3   use 2700 baud as output baudrate\n"
         " -4   use 3600 baud as output baudrate\n"	
         " -b   define output baudrate\n"
         " -f   define output frecuency\n"	 	  	   	      
